...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><img className="brand-badge" src={`${import.meta.env.BASE_URL}torras-mark.jpeg`} alt="TORRAS" /><div><strong>TORRAS</strong><small>拍剪协作台</small></div></div>
    ...
    <main>
      ...
      {!ready ? <Loading /> : <>
        ...
        {tab === "requests" && <RequestsView requests={data.requests} onNew={() => setRequestOpen(true)} onStatus={(id, status) => setData((current) => ({ ...current, requests: current.requests.map((item) => item.id === id ? { ...item, status } : item) }))} onDelete={(id) => { setData((current) => ({ ...current, requests: current.requests.filter((item) => item.id !== id) })); showToast("success", "需求已删除"); }} />}
...
function ProgressView({ data, totals, setData, updateProduct, addProduct, switchWeek }: { data: AppData; totals: { done: number; scheduled: number; remaining: number; capacityPct: number }; setData: React.Dispatch<React.SetStateAction<AppData>>; updateProduct: (id: string, patch: Partial<Product>) => void; addProduct: () => void; switchWeek: (start: string) => void }) {
  return <>
    ...
    <section className="deliveries"><div className="section-heading"><div><span className="section-index">DELIVERY BOARD</span><h2>视频进度明细</h2><p>修改目标和完成量后，进度与状态会自动更新</p></div><button className="primary-button" onClick={addProduct}>＋ 新增产品行</button></div>
